...
6. Specs under `specs/*.md` and Keyword Specs under `specs/keywords/` describe **Target only**. Skills under `.agents/skills/` are procedures.
7. Command syntax: `specs/keywords/` is the **only** keyword tree. Skip `archive/` for new commands. Honor `deprecated`.
8. Help Dump for Target: see `versions.md`. Open bugs: `specs/ma-bugs.md`.
9. Do not write plugin or macro files until the user gives an absolute `gma3_library` path.
10. Do not guess station IPs. Default OSC to `127.0.0.1` unless the user names a host.
